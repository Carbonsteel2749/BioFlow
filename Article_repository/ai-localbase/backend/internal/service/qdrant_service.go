...
func (s *QdrantService) ScrollPointPayloads(ctx context.Context, knowledgeBaseID string) ([]QdrantStoredPoint, error) {
	return s.scrollPointPayloads(ctx, knowledgeBaseID, nil)
}

// ScrollPointPayloadsByFilter 按 filter 拉取向量库中匹配的 point payload。
func (s *QdrantService) ScrollPointPayloadsByFilter(ctx context.Context, knowledgeBaseID string, filter map[string]any) ([]QdrantStoredPoint, error) {
	return s.scrollPointPayloads(ctx, knowledgeBaseID, filter)
}

func (s *QdrantService) scrollPointPayloads(ctx context.Context, knowledgeBaseID string, filter map[string]any) ([]QdrantStoredPoint, error) {
	if !s.IsEnabled() {
		return nil, nil
	}
...
func (s *QdrantService) querySparse(ctx context.Context, knowledgeBaseID string, vector SparseVector, limit int, filter map[string]any) ([]QdrantSearchResult, error) {
	return s.querySparseUsing(ctx, knowledgeBaseID, qdrantSparseVectorName, vector, limit, filter)
}

func (s *QdrantService) querySparseUsing(ctx context.Context, knowledgeBaseID, using string, vector SparseVector, limit int, filter map[string]any) ([]QdrantSearchResult, error) {
...
// SearchHybrid 执行 dense + BM25 + sparse 三路检索，并使用 RRF 融合排名。
func (s *QdrantService) SearchHybrid(ctx context.Context, params HybridSearchParams) ([]SearchResult, error) {
	...
	if len(params.BM25Vector.Indices) > 0 && len(params.BM25Vector.Values) > 0 {
		wg.Add(1)
		go func() {
			defer wg.Done()
			results, err := s.querySparseUsing(ctx, params.CollectionName, qdrantBM25VectorName, params.BM25Vector, searchLimit, filter)
			if err != nil {
				bm25Err = err
				return
			}
			bm25Results = applyScoreThreshold(results, float64(params.ScoreThreshold))
		}()
	}
	...
	if len(bm25Results) == 0 && len(sparseResults) == 0 {
		return denseResults, nil
	}

	return rrfFusionMulti([][]SearchResult{denseResults, bm25Results, sparseResults}, []string{qdrantDenseVectorName, qdrantBM25VectorName, qdrantSparseVectorName}, topK), nil
}

// rrfFusion 使用 RRF 融合
func rrfFusion(denseResults []SearchResult, sparseResults []SearchResult, topK int) []SearchResult {
	return rrfFusionMulti([][]SearchResult{denseResults, sparseResults}, []string{qdrantDenseVectorName, qdrantSparseVectorName}, topK)
}

func rrfFusionMulti(resultSets [][]SearchResult, channelNames []string, topK int) []SearchResult {
	const k = 60.0
	if topK <= 0 {
		topK = 5
	}
	...
	ranks := make([]map[string]int, len(resultSets))
	for index := range ranks {
		ranks[index] = make(map[string]int)
	}
	channels := make(map[string]map[string]struct{})
	addResults := func(results []SearchResult, channel string, channelRanks map[string]int) {
		for idx, item := range results {
			rank := float64(idx + 1)
			scores[item.ID] += 1.0 / (k + rank)
			channelRanks[item.ID] = idx + 1
	...
	for index, results := range resultSets {
		if index < len(channelNames) {
			addResults(results, channelNames[index], ranks[index])
		}
	}
	...
	for id, score := range scores {
		...
		payload[qdrantPayloadDenseRank] = ranks[0][id]
		payload[qdrantPayloadSparseRank] = ranks[len(ranks)-1][id]
...
func qdrantPointVectors(dense []float64, sparse SparseVector) any {
	return qdrantPointVectorsWithSparse(dense, sparse, sparse)
}

func qdrantPointVectorsWithSparse(dense []float64, bm25 SparseVector, sparse SparseVector) any {
	...
	return map[string]any{
		qdrantDenseVectorName: dense,
		qdrantBM25VectorName: map[string]any{
