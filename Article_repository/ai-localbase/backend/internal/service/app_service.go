...
func NewAppService(qdrant *QdrantService, store *AppStateStore, chatHistory ChatHistoryStore, serverConfig model.ServerConfig) *AppService {
	...
	if strings.TrimSpace(serverConfig.CrossEncoderURL) != "" {
		service.reranker = NewCrossEncoderReranker(
			serverConfig.CrossEncoderURL,
			serverConfig.CrossEncoderModel,
			serverConfig.CrossEncoderAPIKey,
		)
	} else {
	...
	service.SetQueryTranslator(NewQueryTranslator(llmService, QueryTranslationConfig{
		Mode:           serverConfig.QueryTranslationMode,
		Enabled:        serverConfig.EnableQueryTranslation,
		Provider:       serverConfig.QueryTranslationProvider,
		BaseURL:        serverConfig.QueryTranslationBaseURL,
		APIKey:         serverConfig.QueryTranslationAPIKey,
		Model:          serverConfig.QueryTranslationModel,
		TimeoutSeconds: serverConfig.QueryTranslationTimeoutSeconds,
	}))
...
func normalizeRetrievalConfig(cfg model.RetrievalConfig, serverConfig model.ServerConfig) model.RetrievalConfig {
	...
	if rerankStrategy == "" {
		...
		} else if serverConfig.EnableSemanticReranker {
...
func validateRetrievalConfig(cfg model.RetrievalConfig) error {
	if normalizeRerankStrategy(cfg.RerankStrategy) == "" {
		return fmt.Errorf("rerank strategy must be keyword, semantic, or cross_encoder")
	}
...
func (s *AppService) GetDocumentDetail(knowledgeBaseID, documentID, focusChunkID string) (model.DocumentDetailResponse, error) {
	...
	// 优先展示向量库中真实入库的 chunk，保证详情与检索结果一致；
	// 向量库不可用或尚未入库时回退到实时分块。
	chunks, ok := s.loadStoredDocumentChunks(knowledgeBaseID, documentID)
	if !ok || len(chunks) == 0 {
...
// loadStoredDocumentChunks 从向量库读回文档真实入库的 chunk（按 chunk_index 升序）。
func (s *AppService) loadStoredDocumentChunks(knowledgeBaseID, documentID string) ([]DocumentChunk, bool) {
	if s == nil || s.qdrant == nil || !s.qdrant.IsEnabled() || strings.TrimSpace(documentID) == "" {
		return nil, false
	}
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	points, err := s.qdrant.ScrollPointPayloadsByFilter(ctx, knowledgeBaseID, documentFilter(documentID))
	if err != nil {
		log.Printf("load stored chunks for document %s failed: %v", documentID, err)
		return nil, false
	}
	...
	for _, point := range points {
		...
		chunks = append(chunks, DocumentChunk{
			ID:              payloadString(point.Payload, "chunk_id", ""),
			KnowledgeBaseID: payloadString(point.Payload, "knowledge_base_id", knowledgeBaseID),
			DocumentID:      payloadString(point.Payload, "document_id", documentID),
			DocumentName:    payloadString(point.Payload, "document_name", ""),
			Text:            text,
			Index:           payloadInt(point.Payload, "chunk_index"),
			Kind:            payloadString(point.Payload, "chunk_kind", "text"),
		})
	}
	if len(chunks) == 0 {
		return nil, false
...
func (s *AppService) IndexDocument(document model.Document) (model.Document, error) {
	...
	chunks := s.rag.BuildDocumentChunksSemantic(context.Background(), document, content, embeddingConfig, s.qdrantVectorSize())
	if len(chunks) == 0 {
...
func (s *AppService) EvaluateRetrieve(req model.ChatCompletionRequest) ([]RetrievedChunk, error) {
	...
	// 检索统一走英文问题：中文（或日/韩文）问题先快速翻译成英文，
	// 翻译失败时回退原文，避免检索中断。
	searchQuery := s.retrievalQueryForSearch(context.Background(), req, query).English

	var queryVector []float64
	embeddingStartedAt := time.Now()
	if !s.queryRewriteEnabledForRequest(req) {
		embedCtx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
		defer cancel()
		vectors, err := s.rag.EmbedTexts(embedCtx, s.resolveEmbeddingConfig(req), []string{searchQuery}, s.qdrantVectorSize())
...
func (s *AppService) DebugRetrieve(req model.RetrievalDebugRequest) (model.RetrievalDebugResponse, error) {
	...
	// 检索使用英文问题；提前解析一次，既能复用翻译缓存，也能在调试结果中回显
	translation := s.retrievalQueryForSearch(context.Background(), chatReq, query)
	...
	translationReason := "检索问题无需翻译，直接使用原问题"
	if translation.Applied {
		translationReason = fmt.Sprintf("已翻译为英文检索：%s", translation.English)
	} else if translation.Reason == queryTranslationReasonFailed || translation.Reason == queryTranslationReasonUnusable {
		translationReason = "翻译失败，已回退使用原问题检索"
	}

	trace := []model.RetrievalDebugTraceStep{{
		Stage:  "query_translation",
		Status: ternaryString(translation.Applied, "ok", "skipped"),
	...
	confidence := buildRetrievalDebugConfidence(translation.English, chunks)
	...
	return model.RetrievalDebugResponse{
		Query:            query,
		EnglishQuery:     translation.English,
		QueryTranslation: translation.Reason,
...
func (s *AppService) debugRetrieveVerbose(req model.ChatCompletionRequest, query string) ([]RetrievedChunk, *model.RetrievalDebugVerboseDetails, []string, error) {
	...
	// 与正式检索保持一致：使用英文问题检索
	searchQuery := s.retrievalQueryForSearch(ctx, req, query).English
	...
	if s.queryRewriteEnabledForRequest(req) {
		...
		rewriteResult, err := s.queryRewriter.Rewrite(ctx, searchQuery, history)
		...
		if err == nil && len(rewriteResult.RewrittenQueries) > 0 {
			...
			queries := limitRetrievalQueries(
				mergeRetrievalQueries([]string{searchQuery}, rewriteResult.RewrittenQueries),
	...
	if len(candidates) == 0 {
		...
		searchStart := time.Now()
		candidates, err = s.collectCandidatesForQueries(ctx, knowledgeBaseIDs, req, queryVector, nil, params.candidateTopK, useHybrid, searchQuery)
		if err != nil {
			return nil, nil, nil, err
		}
		verboseDetails.VectorSearchMs = time.Since(searchStart).Milliseconds()
	...
	verboseDetails.TopCandidates = convertToDebugChunks(candidates, searchQuery, 5)
	...
	if s.shouldBypassRerank(candidates) {
		...
		afterRerank = s.rerankCandidates(ctx, candidates, searchQuery, req)
		verboseDetails.RerankMs = time.Since(rerankStart).Milliseconds()
	}
	verboseDetails.AfterRerankCount = len(afterRerank)
	verboseDetails.TopAfterRerank = convertToDebugChunks(afterRerank, searchQuery, 5)
	...
	verboseDetails.TopAfterMMR = convertToDebugChunks(selected, searchQuery, 5)
	verboseDetails.MMREffect = analyzeMMREffect(beforeMMR, selected, searchQuery)
...
func (s *AppService) SetQueryTranslator(translator QueryTranslator) {
	s.queryTranslator = translator
	if setter, ok := translator.(interface {
		SetChatConfigProvider(func() model.ChatModelConfig)
	}); ok {
		setter.SetChatConfigProvider(s.currentChatConfig)
	}
...
// retrievalQueryForSearch 解析用于知识库检索的问题。
// 判断机制：问题含中文（或日文/韩文）时先翻译为英文再检索；
// 纯英文问题直接透传；翻译失败或结果不可用时回退原文，保证检索不中断。
func (s *AppService) retrievalQueryForSearch(ctx context.Context, req model.ChatCompletionRequest, rawQuery string) QueryTranslation {
	if s == nil || s.queryTranslator == nil {
		return QueryTranslation{
			Original: rawQuery,
			English:  rawQuery,
			Reason:   queryTranslationReasonDisabled,
		}
	}
	if setter, ok := s.queryTranslator.(interface {
		SetChatConfigProvider(func() model.ChatModelConfig)
	}); ok {
		setter.SetChatConfigProvider(func() model.ChatModelConfig {
			return s.resolveChatConfig(req)
	...
	startedAt := time.Now()
	translation := s.queryTranslator.TranslateToEnglish(ctx, rawQuery)
	if translation.Reason == queryTranslationReasonEmpty ||
		translation.Reason == queryTranslationReasonDisabled ||
		translation.Reason == queryTranslationReasonNotNeeded {
		return translation
	}

	logRetrievalStageMetrics(req, rawQuery, "query_translation", startedAt, map[string]any{
		"status":        translation.Reason,
		"applied":       translation.Applied,
		"cache_hit":     translation.CacheHit,
...
func (s *AppService) upsertDocumentChunks(knowledgeBaseID string, chunks []DocumentChunk, vectors [][]float64) error {
	...
	bm25Vectors := make([]SparseVector, len(chunks))
	sparseVectors := make([]SparseVector, len(chunks))
	texts := make([]string, 0, len(chunks))
	for index, chunk := range chunks {
		bm25Vectors[index] = BuildSparseVector(chunk.Text)
		texts = append(texts, chunk.Text)
	}
	copy(sparseVectors, bm25Vectors)
	if strings.TrimSpace(s.serverConfig.SparseEncoderURL) != "" {
		encoded, err := s.rag.EncodeSparseTexts(
			context.Background(),
			s.serverConfig.SparseEncoderURL,
			s.serverConfig.SparseEncoderModel,
			s.serverConfig.SparseEncoderAPIKey,
			texts,
		)
		if err == nil && len(encoded) == len(chunks) {
			sparseVectors = encoded
		} else if err != nil {
			log.Printf("BGE-M3 sparse encoder unavailable, using lexical fallback: %v", err)
		}
	}
	for index, chunk := range chunks {
		...
		points = append(points, QdrantPoint{
			...
			Vector: qdrantPointVectorsWithSparse(vector, bm25Vectors[index], sparseVectors[index]),
...
func (s *AppService) retrieveRelevantChunks(req model.ChatCompletionRequest, queryVector []float64) ([]RetrievedChunk, error) {
	...
	if s.queryRewriteEnabledForRequest(req) {
		...
		if err != nil {
			...
			candidates = s.filterRetrievedChunksToScope(req, knowledgeBaseIDs, candidates)
			selected := s.applySelectionStrategy(req, searchQuery, ctx, candidates, params)

			if autoExpand && strings.TrimSpace(req.DocumentID) == "" && isLowConfidenceSelection(searchQuery, selected) {
				...
				if len(expandedCandidates) > 0 {
					...
					expandedSelected := s.applySelectionStrategy(req, searchQuery, ctx, expandedCandidates, expandedParams)
			...
			if s.semanticCache != nil && len(queryEmbedding) > 0 {
				s.semanticCache.Set(cacheScope, queryEmbedding, searchQuery, selected)
			}
	...
	candidates, err := s.collectCandidatesForQueries(ctx, knowledgeBaseIDs, req, queryVector, searchQueries, params.candidateTopK, useHybrid, searchQuery)
	if err != nil {
		return nil, err
	}
	candidates = s.filterRetrievedChunksToScope(req, knowledgeBaseIDs, candidates)
	...
	if autoExpand && strings.TrimSpace(req.DocumentID) == "" && isLowConfidenceSelection(searchQuery, selected) {
		...
		expandedCandidates, err := s.collectCandidatesForQueries(ctx, knowledgeBaseIDs, req, queryVector, searchQueries, expandedCandidateTopK, expandUseHybrid, searchQuery)
		if err == nil {
			expandedCandidates = s.filterRetrievedChunksToScope(req, knowledgeBaseIDs, expandedCandidates)
			expandedParams := params
			expandedParams.perDocumentLimit++
...
func (s *AppService) collectCandidates(knowledgeBaseIDs []string, req model.ChatCompletionRequest, queryVector []float64, candidateTopK int, useHybrid bool, query string) ([]RetrievedChunk, error) {
	...
	// 尝试从查询中提取文件名并找到对应文档。
	// 文件名检测使用用户原始问题：翻译可能改写文件名，导致无法定位文档。
	var filenameDetectedDocID string
	if strings.TrimSpace(req.DocumentID) == "" {
		filenameSourceQuery := query
		if original := latestUserMessage(req.Messages); strings.TrimSpace(original) != "" {
			filenameSourceQuery = original
		}
		filenames := extractFilenamesFromQuery(filenameSourceQuery)
	...
	for _, knowledgeBaseID := range knowledgeBaseIDs {
		...
		if useHybrid {
			...
			bm25Vector := BuildSparseVector(query)
			sparseVector := bm25Vector
			if strings.TrimSpace(s.serverConfig.SparseEncoderURL) != "" {
				encoded, encodeErr := s.rag.EncodeSparseTexts(context.Background(), s.serverConfig.SparseEncoderURL, s.serverConfig.SparseEncoderModel, s.serverConfig.SparseEncoderAPIKey, []string{query})
				if encodeErr == nil && len(encoded) == 1 {
					sparseVector = encoded[0]
				} else if encodeErr != nil {
					log.Printf("BGE-M3 sparse query unavailable, using lexical fallback: %v", encodeErr)
				}
			}
			searchResults, err := s.rag.SearchHybridThreeWay(context.Background(), s.qdrant, knowledgeBaseID, queryVector, bm25Vector, sparseVector, candidateTopK, filter)
			if err != nil {
...
func (s *AppService) rerankCandidates(ctx context.Context, candidates []RetrievedChunk, query string, req model.ChatCompletionRequest) []RetrievedChunk {
	...
	strategy := s.rerankStrategyForRequest(req)
	if s != nil && (strategy == "semantic" || strategy == "cross_encoder") && s.reranker != nil {
...
func normalizeRerankStrategy(strategy string) string {
	...
	case "semantic", "embedding":
		return "semantic"
	case "cross_encoder", "cross-encoder", "crossencoder":
