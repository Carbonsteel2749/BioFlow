...
// SplitTextIntoParagraphSentences 把文本切成「段落 → 句子」两级结构，
// 供上层基于 embedding 相似度做语义分块。句子边界规则与 SemanticChunk 完全一致，
// 保证不会切断句子。
func SplitTextIntoParagraphSentences(text string) [][]string {
	cleaned := normalizeChunkText(text)
	if cleaned == "" {
		return nil
	}
	...
	for _, paragraph := range paragraphs {
		// PDF/网页提取的正文常带行内硬换行，这些换行并不代表句子结束。
		// 段落内的单换行统一归一为空格，只按句末标点切分，避免把句子切成碎片。
		flattened := strings.Join(strings.Fields(strings.ReplaceAll(paragraph, "\n", " ")), " ")
		if flattened == "" {
			continue
		}
		sentences := splitSentences(flattened, false)
		...
		if len(cleanedSentences) > 0 {
			result = append(result, cleanedSentences)
		}
	}
	return result
}

// RuneCount 返回文本的 rune（字符）数量，用于分块的长度约束判断。
func RuneCount(text string) int {
