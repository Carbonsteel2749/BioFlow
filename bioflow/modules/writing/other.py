...
class OtherModule(Module):
    spec = ModuleSpec(
        name="other",
        kind="writing",
        deps=["abstract", "discussion"],
        description="Generate additional sections (figures, acknowledgements, etc.).",
    )

    def run(self, context: RunContext, module_input: ModuleInput) -> ModuleOutput:
        summary = "Other sections draft generated."
        if context.model_router and module_input.payload.get("use_llm"):
            prompt = "Write supporting sections such as figure legends and acknowledgements."
            summary = context.model_router.generate(prompt)

        artifact_dir = context.artifacts_dir / "other"
