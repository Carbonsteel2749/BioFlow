...
class IntroductionModule(Module):
    spec = ModuleSpec(
        ...
        kind="writing",
        deps=[],
        description="Generate introduction draft.",
    )

    def run(self, context: RunContext, module_input: ModuleInput) -> ModuleOutput:
        summary = "Introduction draft generated."
        if context.model_router and module_input.payload.get("use_llm"):
            prompt = "Write an introduction with background and rationale."
            summary = context.model_router.generate(prompt)

        artifact_dir = context.artifacts_dir / "introduction"
